...
class GW_Go_Portfolio {
	...
	public function ajax_load_portfolio() {
		...
		if ( $portfolio_id ) {
			echo do_shortcode('[go_portfolio id="' . esc_attr( $portfolio_id ) . '" current_page="' . esc_attr( $current_page ) . '" exclude_posts="' . esc_attr( isset( $post->ID ) && !empty( $post->ID ) ? $post->ID : '' ) . '"]');	
		}
		exit;
	}
			...
			if ( !empty( $portfolios ) ) {
					...
					if ( $portfolio['id'] == $id ) {
							...
							if ( isset( $post_types[$portfolio['post-type']] ) ) {
								...
								/* Modify query args - exlude current post */
								if ( is_singular() && isset( $portfolio['exclude-current'] ) ) {
									if ( isset( $post ) && isset( $post->post_type ) && isset( $post->ID ) ) {
								...
								<div id="<?php echo esc_attr( self::$plugin_prefix . '_' . $portfolio['id'] ); ?>" style="<?php echo esc_attr( ( isset( $margin_top ) ? 'margin-top:' . $margin_top . ';' : '' ) . ( isset( $margin_bottom ) ? 'margin-bottom:' . $margin_bottom . ';' : '' ) ); ?>">
									...
									<div class="gw-gopf-posts-wrap">
										...
										<div class="gw-gopf-slider-controls-wrap gw-gopf-clearfix<?php echo ( isset( $portfolio['slider-arrows-align'] ) && !empty( $portfolio['slider-arrows-align'] ) ?  ' '. $portfolio['slider-arrows-align'] : '' ); ?>">
											<div class="gw-gopf-slider-controls gw-gopf-clearfix">
												<div class="gw-gopf-control-prev"><a href="#"><img src="<?php echo GW_GO_PORTFOLIO_URI . 'assets/images/icon_prev.png'; ?>" class="gw-gopf-retina" alt=""></a></div>
												<div class="gw-gopf-control-next"><a href="#"><img src="<?php echo GW_GO_PORTFOLIO_URI . 'assets/images/icon_next.png'; ?>" class="gw-gopf-retina" alt=""></a></div>
											</div>
										</div>
										<?php endif; ?>							
										<div class="gw-gopf-posts-wrap-inner">
											<div class="gw-gopf-posts gw-gopf-clearfix"<?php echo ( isset( $portfolio['column-layout'] ) && !empty( $portfolio['column-layout'] ) ? ' data-col="' . preg_replace('/[^0-9]/', '', $portfolio['column-layout'] ) . '"' : '' ); ?><?php echo ( $layout_type == 'slider' ? ' data-slider="' . esc_js( json_encode( $slider_data ) ) . '"' : '' ); ?>>
												...
												$post_terms = get_the_terms( $post->ID, $filter_tax ); 
												...
												<div id="<?php echo esc_attr( $post->ID ); ?>" class="gw-gopf-col-wrap" data-filter="<?php echo esc_attr( implode(' ', $post_term_list ) ); ?>">
													...
													if ( $lighbox_type == 'image' ) {
														...
														if ( isset( $portfolio['overlay-style'] ) && $portfolio['overlay-style'] == '2' ) {
															...
															$button_content = '<img src="' . GW_GO_PORTFOLIO_URI . 'assets/images/icon_large.png" class="gw-gopf-retina" alt="">';
														}
														...
														if ( isset( $portfolio['overlay-style'] ) && $portfolio['overlay-style'] == '2' ) {
															...
															if ( $lighbox_type == 'video' ) {
																$button_content = '<img src="' . GW_GO_PORTFOLIO_URI . 'assets/images/icon_video.png" class="gw-gopf-retina" alt="">';
															} else {
																$button_content = '<img src="' . GW_GO_PORTFOLIO_URI . 'assets/images/icon_audio.png" class="gw-gopf-retina" alt="">';
															}
														}
													...
													if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
														...
														$sale_price = '';
														$regular_price = '';
														$template_data['woo_price'] = '';
														if ( $woo_is_variation && $woo_product->product_type == 'variable' ) { 
															$sale_price = isset( $post_meta['_min_variation_sale_price'][0] ) && !empty( $post_meta['_min_variation_sale_price'][0] ) ? '<ins>' . woocommerce_price( $post_meta['_min_variation_sale_price'][0] ) . '</ins>' : '';
															$regular_price = isset( $post_meta['_min_variation_regular_price'][0] ) && !empty( $post_meta['_min_variation_regular_price'][0] ) ? ( $sale_price ? '<del>' . woocommerce_price( $post_meta['_min_variation_regular_price'][0] ) . '</del>' :  woocommerce_price( $post_meta['_min_variation_regular_price'][0] ) ) : '';
															$template_data['woo_price'] = '<span class="gw-gopf-woo-from">' . __( 'from: ', 'go_portfolio_textdomain' ) . '</span>' . $regular_price . $sale_price;
														} else {															
															$sale_price = isset( $post_meta['_sale_price'][0] ) && !empty( $post_meta['_sale_price'][0] ) ? '<ins>' . woocommerce_price( $post_meta['_sale_price'][0] ) . '</ins>' : '';
															$regular_price = isset( $post_meta['_regular_price'][0] ) && !empty( $post_meta['_regular_price'][0] ) ? ( $sale_price ? '<del>' . woocommerce_price( $post_meta['_regular_price'][0] ) . '</del>' : woocommerce_price( $post_meta['_regular_price'][0] ) ) : '';
									...
									?>
									<div class="gw-gopf-pagination-wrapper gw-gopf-clearfix<?php echo ( isset( $portfolio['pagination-align'] ) && !empty( $portfolio['pagination-align'] ) ?  ' '. $portfolio['pagination-align'] : '' ); ?>" data-posts="<?php echo esc_attr( $posts_count );?>" data-pages="<?php echo esc_attr( $pages_count );?>" data-current-page="<?php echo esc_attr( $current_page );?>" data-current-id="<?php echo $post->ID; ?>">
								...
								<div class="gw-gopf-filter gw-gopf-clearfix <?php echo ( isset( $portfolio['filter-align'] ) && !empty( $portfolio['filter-align'] ) ?  ' '. $portfolio['filter-align'] : '' ); ?>">
									<div class="gw-gopf-cats">
										...
										?><span data-filter="<?php echo esc_attr( $current_term->slug ); ?>" class="<?php echo isset( $filter_terms ) && is_array( $filter_terms ) && in_array($current_term->term_id, $filter_terms) ? '' :'gw-gopf-hidden' ?>"><a href="#"<?php echo ( isset( $portfolio['filter-tag-style'] ) && !empty( $portfolio['filter-tag-style'] ) ? 'class="' . $portfolio['filter-tag-style'] . '"' : '' ); ?>><?php echo $current_term->name; ?></a></span><?php 
										endforeach;		
										?>
									</div>
								</div>
								...
								/* /Portfolio filter */
								?>
								<?php echo $portfolio_posts_content; ?>								
								</div>								
								</div>															
								<?php
